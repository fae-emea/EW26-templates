...
#ifdef TEST_011_TEMPERATURE_FLOAT

#define MAX_TEST	1000

static float	vtotempk(float);
static float	temptovk(float);
static	float	vtotemp_cnv(float,const int *,int);
...
static	float	vtotemp_cnv(float a,const int *i,int j)
{
  const int	*k,*l;
  int	m,n,o;
  k=i;
  ...
  for(n=0;n<5;n++)
  {
    if((float)(*k)<=a && a<=(float)( *l))
    {
      ...
      for(o=0;o<10;o++)
      {
        if((float)(*k)<=a && a<=(float)(*l))
        {
          return((a-(float)(*k))*(1.0/((float)(*l)-(float)(*k)))+(float)m);
