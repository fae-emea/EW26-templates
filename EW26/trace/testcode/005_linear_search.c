...
#ifdef TEST_005_LINEARSEARCH

#define MAX_TEST	10000	

#define	TEST_DATA_512BYTE		(128)		// 4*128=512Byte

const unsigned long data_512byte_0[TEST_DATA_512BYTE];
const unsigned long data_512byte_1[TEST_DATA_512BYTE];	
	
unsigned long linear_search(unsigned long x,const unsigned long *a, unsigned long num);
...
int test_005(void)
{
	...
	for(j=0;j<MAX_TEST;j++){
		ans = linear_search(0x6e41b303ul,data_512byte_0, TEST_DATA_512BYTE);
		if (ans != 0x7f) {
			for (;;);
		}
		ans = linear_search(0xb8d0f8caul, data_512byte_0, TEST_DATA_512BYTE);
		if (ans != 0x0) {
			for (;;);
		}
		ans = linear_search(0x57ba44cdul, data_512byte_0, TEST_DATA_512BYTE);
		if (ans != 64) {
			for (;;);
		}
		ans = linear_search(0x00ul, data_512byte_0, TEST_DATA_512BYTE);
		if (ans != 0xfffffffful) {
			for (;;);
		}
		ans = linear_search(0x929cb50bul, data_512byte_1, TEST_DATA_512BYTE);
		if (ans != 0x0) {
			for (;;);
		}
		ans = linear_search(0x8ee1504bul, data_512byte_1, TEST_DATA_512BYTE);
		if (ans != 64) {
			for (;;);
		}
		ans = linear_search(0x2f99ab0eul, data_512byte_1, TEST_DATA_512BYTE);
		if (ans != 0x7f) {
			for (;;);
		}
		ans = linear_search(0x00ul, data_512byte_1, TEST_DATA_512BYTE);
		if (ans != 0xfffffffful) {
			for (;;);
		}
	}
